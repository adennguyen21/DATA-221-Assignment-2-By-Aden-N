# Question 1 - Reading and processing a text file.
# =============================================================
...
def calculate_word_frequency(word_list): # Calculates the frequency of each word.
    word_frequencies = {}

    for word in word_list: # Traverses through every word in the cleaned up word list.
        word_count = 0

        for comparing_word in word_list: # Compares the word from the outer loop to this inner loop word.
            if word == comparing_word:
                word_count += 1 # Count goes up for every word that matches with the word from the outer loop.

        word_frequencies[word] = word_count # Adds the word as a key, and the frequency of that word as the value.
...
def print_top_10_most_frequent_words(word_frequencies): # Prints out the top ten most frequent words.
    top_10_count = 0 # Once this count reaches 10, this function stops printing.

    for item in word_frequencies.items(): # Goes through each element in the frequencies' library.
        if top_10_count < 10:
            word, frequency = item # Separates the word (key) and frequency number (value) as two different variables.
            print(f"{word} -> {frequency}") # Prints out the result.
            top_10_count += 1
...
def main(): # Main function to call the other functions in.
    ...
    word_frequencies = calculate_word_frequency(finished_word_list)

    print_top_10_most_frequent_words(word_frequencies)
